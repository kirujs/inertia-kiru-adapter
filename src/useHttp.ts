...
import { ref, signal, Signal, ComputedSignal } from 'kiru'
...
export function useHttp<TForm extends FormDataType<TForm>, TResponse = unknown>(
  ...
  const excludeKeysRef = ref<FormDataKeys<TForm>[]>([])
  const pendingOptimisticRef = ref<((currentData: TForm) => Partial<TForm>) | null>(null)
  ...
  const submit = async (method: Method, url: string, options: UseHttpSubmitOptions<TResponse, TForm>): Promise<TResponse> => {
  ...
  const submitWithArgs = (...args: UseHttpSubmitArguments<TResponse, TForm>): Promise<TResponse> => {
